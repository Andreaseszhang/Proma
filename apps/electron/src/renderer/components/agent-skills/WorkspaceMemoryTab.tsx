...
function buildWorkspaceMemoryInitPrompt(historyRange: MemoryHistoryRange): string {
  ...
  return `请为当前项目初始化并沉淀长期记忆。这里的“项目”指系统提示中的“项目根目录”及其关联的 Agent 工作会话；不要把 Proma 内部工作区笼统当作项目。
...
- 如果系统提示表明当前项目是本地项目，“项目根目录”就是实际 cwd。允许从其“工作区级 Context”及明确关联的长期项目资料读取证据；不要自动读取、创建或修改该目录内的 \`.claude/\`、\`CLAUDE.md\`、MCP 或 Skills 配置，除非用户明确要求。
...
- 系统提示中的“工作区级 Context”与项目级长期资料用于跨会话保留调研、架构分析和项目知识。先区分它们与会话级临时产物，再决定可作为长期记忆证据的内容。

沉淀目标：
1. 从允许读取的会话和 Context 中提炼稳定的项目知识：项目结构、常用命令、架构边界、可靠决策、踩坑经验、用户偏好，以及未来 Agent 必须注意的事项。不要把聊天流水账、单次调试过程或当前任务的临时产物当作长期知识。
2. 只更新系统提示明确给出的“工作区 CLAUDE.md”绝对路径。这里是 Proma 管理的项目指令文件；内容仅限稳定、跨会话有效的项目规则、入口和工作方法，不得混入临时调试、聊天记录或长篇资料。
3. 只更新系统提示明确给出的“工作区 Auto Memory 目录”中的 \`MEMORY.md\`、必要的主题文件和 \`user-profile.md\`，不要在其他目录创建记忆文件。\`MEMORY.md\` 保持简短的主题索引与路由，主题细节拆分到主题文件。
...
3. 优先小幅、可审阅的增量更新：CLAUDE.md 保持精炼，MEMORY.md 不承载长正文，跨会话的长资料仍留在项目级长期资料或工作区级 Context。
