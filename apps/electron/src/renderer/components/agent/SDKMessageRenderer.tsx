...
function UserInputMessage({ message }: { message: SDKUserMessage }): React.ReactElement {
  ...
  return (
    <Message from={useModernBubbleStyle ? 'user' : 'assistant'} className={useModernBubbleStyle ? 'group/user-message' : undefined}>
      ...
      {text && (
        <MessageActions className={useModernBubbleStyle ? 'mt-0.5 self-end justify-end' : 'pl-[46px] mt-0.5'}>
          {useModernBubbleStyle && (meta.createdAt || isScheduledRun) && (
            <span className="flex items-center gap-2 leading-none opacity-0 transition-opacity duration-150 group-hover/user-message:opacity-100">
              {meta.createdAt && (
                <span className="message-time text-[10px] text-foreground/[0.38]">{formatMessageTime(meta.createdAt)}</span>
              )}
              {isScheduledRun && (
