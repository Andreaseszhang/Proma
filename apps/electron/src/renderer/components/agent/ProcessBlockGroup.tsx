...
export function getStreamingTextPreview(text: string): string {
  if (!text.trim()) return ''
  const chars = Array.from(text)
  const tail = chars.length > MAX_STREAMING_PREVIEW_CHARS
    ? chars.slice(-MAX_STREAMING_PREVIEW_CHARS).join('')
    : text
  const preview = tail.trim()
  return chars.length > MAX_STREAMING_PREVIEW_CHARS ? `…${preview}` : preview
}
...
export function getStreamingTurnPreview(blocks: SDKContentBlock[]): string {
  for (let index = blocks.length - 1; index >= 0; index--) {
    ...
    return getStreamingTextPreview(text)
  }
  return ''
}
