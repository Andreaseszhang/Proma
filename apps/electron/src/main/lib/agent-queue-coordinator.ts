...
export class AgentQueueCoordinator {
  ...
  private readonly nextRunGeneration = new Map<string, number>()
  ...
  private tryDispatch(sessionId: string): void {
    ...
    const runGeneration = (this.nextRunGeneration.get(sessionId) ?? 0) + 1
    this.nextRunGeneration.set(sessionId, runGeneration)
