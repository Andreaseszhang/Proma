...
function buildWorkspaceMemoryInitPrompt(historyRange: MemoryHistoryRange): string {
  ...
  return `请帮我初始化并沉淀当前项目的长期记忆。

目标：
1. 读取当前项目${rangeLabel}的 Agent 工作会话，优先关注最新、最有代表性、用户实际完成工作的会话。如果证据不足，请说明而不是编造。
2. 同时检查会话级 Context（Proma 会话工作台目录下的 \`.context/\`）和工作区级 Context（系统提示中“工作区级 Context”给出的绝对路径及相关本地文档），区分当前任务临时产物与跨会话长期资料。
3. 从这些会话和 Context 中提炼项目级别的稳定知识，包括项目结构、常用命令、架构约定、用户偏好、踩坑经验、重要决策和未来 Agent 必须知道的注意事项。
4. 只更新系统提示中标为“Proma 工作区 CLAUDE.md”的绝对路径：只写稳定、跨会话有价值的项目指令和工作方式，避免写临时过程和聊天流水账。
5. 只更新系统提示中标为“工作区 Auto Memory 目录”的绝对路径下的 MEMORY.md 与主题文件：MEMORY.md 只放主题索引和路由，详细内容拆到主题文件；只记录 SDK auto memory 应该长期回忆的经验。
6. 沉淀并持续迭代一份「用户画像」记忆，写入上一步 Auto Memory 目录中的 user-profile.md（并在 MEMORY.md 索引中登记）。这份画像用于让未来的 Agent 越来越懂用户，应包含：
   - 用户的角色、技术背景与擅长领域
...
范围边界：
- 系统提示会给出 Proma 工作区、项目根目录、会话工作台、两级 Context、Proma 工作区 CLAUDE.md 与 Auto Memory 的绝对路径；必须按这些路径操作，不能自行猜测。
- 会话工作台中的 \`.claude/settings.json\`（如存在）是 Proma 托管的 Claude runtime sidecar，不是项目文件或记忆；不要读取、创建或修改它。
- 如果系统提示标明项目根目录是用户选择的本地目录，不要自动读取、创建或修改该目录内的 \`.claude/\`、\`CLAUDE.md\`、MCP 或 Skills 配置，除非用户明确要求。

要求：
- 先查看当前项目可用的会话和文件（包括已有的 user-profile.md），再决定如何写。
