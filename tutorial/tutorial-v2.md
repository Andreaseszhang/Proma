...
**附加目录和文件**是额外授权给当前会话或整个项目的既有资料，适合参考库、素材库、Vault 等不想复制进项目根目录的内容。附加目录不是项目根目录，也不会自动成为 Agent 的默认工作目录。
...
例如，你可以把 Vault 的原始目录作为附加目录，让 Proma 读取或编辑其中的内容；如果它是当前项目长期工作的核心目录，则更适合通过“从本地文件夹创建项目”把它设为项目根目录。
