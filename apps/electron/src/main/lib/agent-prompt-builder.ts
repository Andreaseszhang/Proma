...
- 会话工作台目录: ${workspacePaths?.sessionDir}（存放当前会话的私有临时文件与会话级 Context；不是 cwd）
...
- **路径边界**：会话工作台目录是 \`${workspacePaths?.sessionDir ?? '当前会话目录'}\`；${agentRuntime === 'claude' ? `Claude runtime 的 \`${workspacePaths?.claudeSessionSettingsPath ?? '.claude/settings.json'}\` 是 Proma 托管的 sidecar 配置，不是项目文件或记忆，绝不要修改。` : ''}项目根始终是 cwd；本地项目时它是用户原始目录，Proma 托管项目时它是共享的项目文件根。不要自动读取、创建或修改其中的 \`.claude/\`、\`CLAUDE.md\`、MCP 或 Skills 配置。无论哪种情况，\`./.claude/memory/\` 都不是工作区 Auto Memory；除非用户明确要求，不要在会话工作台或本地项目根目录下创建或更新 \`.claude/memory/\`
