/**
 * ImportSkillDialog — 批量导入 Skill 到当前工作区
 *
 * 进入页面后可通过两种来源批量导入：
 * 1. 其他项目：勾选多个 Skill（自动过滤已安装的同名项），一键批量导入到当前项目。
 * 2. 本地文件夹：拖拽多个 Skill 目录（或 SKILL.md 文件）到拖放区，或通过系统多选
 *    文件夹对话框选择，可配置重复处理策略（跳过 / 覆盖），一键批量导入。
 *
 * 导入完成后展示 成功/跳过/失败 汇总与逐条原因。
 */
...
export function ImportSkillDialog({
  ...
  const installedHint = React.useMemo(
    () => localPaths.filter((p) => installedSlugs.has(basenamePath(p))).length,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl gap-0 overflow-hidden p-0">
        <DialogHeader className="px-6 pb-4 pt-6">
          ...
          <DialogDescription>
            一次把多个 Skill 导入到当前项目。可从其他项目勾选多个，也可从本地文件夹拖入多个 Skill 目录。
          </DialogDescription>
        </DialogHeader>

        <Tabs value={tab} onValueChange={(v) => setTab(v as 'workspace' | 'local')} className="px-6">
          ...
          <TabsContent value="workspace">
            ...
            <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-border/60 bg-background/95 px-6 py-4">
              ...
              >
                ...
                {importingWorkspace ? '导入中...' : `一键导入所选（${selectedCount}）`}
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="local">
            ...
            <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-border/60 bg-background/95 px-6 py-4">
              ...
              <Button size="sm" onClick={() => void handleImportLocal()} disabled={importingLocal || localPaths.length === 0}>
                ...
                {importingLocal ? '导入中...' : `一键导入（${localPaths.length}）`}
              </Button>
            </div>
          </TabsContent>
        </Tabs>
