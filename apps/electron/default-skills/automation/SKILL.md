...
- 会话模式（`sessionMode`）：默认 `new`，每次触发都新建独立子会话；`reuse` 复用上次子会话保留上下文。选择策略见下文。
...
- `new`（默认）：每次触发新建独立子会话。适合**输入稳定、上下文不需要累积**的任务——监控、报告、对账、检查。绝大多数定时任务用这个。
- `reuse`：复用上次创建的子会话，保留对话历史。适合**需要长期跟进同一对象、且每次都要在前次结果上继续**的任务——持续追踪某项目进展、迭代优化某个文档、跟进某个长期 issue。首次运行时会自动新建。

`reuse` 的代价：上下文会随运行次数增长，长任务可能因为 token 累积导致成本上升或触发自动压缩。如果任务每次产出独立（如"看一下今天有什么新 PR"），就别用 `reuse`。
